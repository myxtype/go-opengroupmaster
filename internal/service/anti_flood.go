...
func (s *Service) RevokeAntiFloodWarnByTGGroupID(tgGroupID, operatorTGUserID, targetTGUserID int64) error {
	group, err := s.repo.FindGroupByTGID(tgGroupID)
	if err != nil {
		return err
	}
	...
	deleted, err := s.repo.DeleteLatestWarnLog(group.ID, target.ID, "anti_flood_warn", "anti_flood_warn_action_applied")
	if err != nil {
		return err
	}
	if !deleted {
	...
	_ = s.repo.CreateLog(group.ID, "anti_flood_warn_revoked", operatorID, target.ID)
	return nil
}
