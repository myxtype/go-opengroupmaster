...
func (h *Handler) sendAntiSpamPanel(bot *tgbotapi.BotAPI, target renderTarget, tgUserID, tgGroupID int64) {
	...
	lines := []string{
		...
		fmt.Sprintf("3. 智能识别: %s（>= %d 删除，>= %d 强惩罚）", onOffWithEmoji(view.SmartDetectEnabled), view.SmartDeleteScore, view.SmartPunishScore),
		fmt.Sprintf("4. 屏蔽频道马甲发言: %s", onOffWithEmoji(view.BlockChannelAlias)),
		fmt.Sprintf("5. 屏蔽来自频道转发: %s", onOffWithEmoji(view.BlockForwardFromChan)),
		fmt.Sprintf("6. 屏蔽来自用户转发: %s", onOffWithEmoji(view.BlockForwardFromUser)),
		...
		fmt.Sprintf("15. 删除提醒: %s", antiFloodAlertDeleteText(view.WarnDeleteSec)),
	}
