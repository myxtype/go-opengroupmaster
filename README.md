...
- 欢迎新成员（支持自定义文案）
- 违禁词检测（支持状态开关、独立惩罚面板、提醒自动删除，默认无预置规则）
- 自动回复（关键词匹配，默认无预置规则）
- 积分系统（签到/发言/邀请积分、抽奖消耗积分、手动加减积分）
- 抽奖流程：面板三步创建（标题→人数→关键词）、关键词参与、`/lottery_draw`
- 词云模块：`gojieba` 分词入库、词云黑名单、手动生成与定时推送
...
4. 在群面板点击按钮：
   ...
   - `☁️ 词云`：状态开关、立即生成、设置定时推送时间、管理黑名单词语
   - `💰 积分系统`：状态开关、签到规则/发言规则/邀请规则三级面板、积分/排行别名、手动加减积分
   - `🤖 新增自动回复`：四步创建
   - 第1步选择触发方式（精准触发 / 包含触发）
   - 第2步输入关键词
   - 第3步输入回复内容（支持换行）
   ...
   - `🌐 群时区`：统一设置群组时区（如 `+8`、`-5`、`+8:30`、`UTC+8`），默认 `UTC+8`
   - `🌙 夜间模式`：状态开关、夜间时段设置（先设置开始小时 `0-23`，再设置结束小时 `0-23`）、处理方式（删除媒体 / 全局禁言）
   - 夜间时段默认：`00:00-08:00`（按群时区生效）
   - `💰 积分系统`：状态开关、签到规则、发言规则、邀请规则、积分/排行别名、手动加减积分
   - 签到规则：可配置签到口令（默认 `签到`）与签到奖励积分（默认 `1`）
   - 发言规则：可配置每条消息奖励积分（默认 `1`）、最小字数限制、每日积分上限
   ...
   - `📊 数据统计`：查看累计/今日/近7日/近30日统计与活跃榜（按群时区口径）
...
- 中文字符验证码会发送一张包含 2 个中文字符的图片，并提供 4 个两字中文选项供选择
...
- 积分系统支持配置：签到奖励、发言奖励、发言每日上限、发言最小字数、邀请奖励、邀请每日上限、签到/积分/排行别名
- 积分系统支持管理员手动加减积分：先输入用户名/用户ID/转发成员消息，再输入积分数值（正整数）
- 参与抽奖会消耗积分（默认每次 `1` 积分），积分不足时会提示
- 词云模块支持：
  - 群消息分词后按“群/日期/用户/词语”持久化统计
  - 管理员可配置每日自动推送时间（时:分，按群时区触发）
  - 管理员可配置词云黑名单词语，生成词云时自动过滤
  - 群内管理员命令 `/wordcloud` 可立即生成并发送今日词云
  - 如配置 `WORDCLOUD_JIEBA_DICT_DIR`，应指向词典目录（自动读取固定文件名）；未配置则使用 gojieba 默认词典路径
  - 如未配置 `WORDCLOUD_FONT_PATH`，程序会尝试系统常见中文字体路径；若都不可用则会提示配置字体
- 群时区支持输入：`+8`、`-5`、`+8:30`、`UTC+8`；夜间时段支持按顺序设置开始/结束小时（均为 `0-23`）
...
- 反垃圾新增“规则 + AI 判定”流程：
  - 先跑规则引擎，强风险直接拦截
  - 支持“编辑消息”二次检测：用户先发正常消息再编辑为垃圾内容时，仍会触发反垃圾处理
